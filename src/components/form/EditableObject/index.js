...
export class Editable extends ApolloEditableObject {
  ...
  renderDeleteButton() {
    ...
    return deletable_object ? (
      <IconButton key="delete" onClick={this.delete} disabled={loading}>
...
class EditableObject extends EditorComponent {
  ...
  renderPanelView(content) {
    return super.renderPanelView(
      content || <div className={'panelEditableObject'}>EditableObject</div>
    )
  }
        ...
        if (definitions && definitions.length) {
          definitions.reduceRight((current, definition) => {
            ...
            if (kind === 'FragmentDefinition' && typeCondition) {
              const needAutoloadFields =
                directives &&
                directives.find(
                  (n) =>
                    n && n.name && n.name.value === 'prismaCmsFragmentAllFields'
                )
                  ? true
                  : false
  ...
  renderChildren() {
    ...
    return (
      <ObjectContext.Consumer key="editable_object">
        {(context) => {
