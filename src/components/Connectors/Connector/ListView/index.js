...
class ListView extends EditorComponent {
  ...
  canBeParent(parent) {
    return (
      ...
      this.findInParent(
        parent,
        (parent) => parent instanceof Connector || parent instanceof NamedField
      )
    )
  }
    ...
    return (
      <ConnectorContext.Consumer key="connector_context">
        {(context) => {
          ...
          const { objects, objectsConnection, ...otherData } = data
         ...
         */
          if (
            ...
            output = (
              <Type {...props} items={items} ObjectContext={ObjectContext}>
                {/* {output} */}
              </Type>
            )
          } else {
